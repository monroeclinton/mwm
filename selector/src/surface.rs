...
impl Surface {
    ...
    pub fn clear_surface(&mut self, config: &Config) {
        let context =
            cairo::Context::new(&self.surface).expect("Unable to find context of surface.");
    ...
    pub fn draw_title(
        ...
        self.surface
            .set_size(config.width as i32, window_height as i32)
        ...
        context.rectangle(0.0, 0.0, title_width, item_height);

        context.fill().expect("Unable to draw input box.");

        set_source_rgb(&context, config.font_active_color);

        context.set_font_face(&font_face);
        context.set_font_size(font_size);
        context.move_to(font_size, (item_height + extents.height) / 2.0);
    ...
    pub fn draw_items(
        ...
        for (i, command) in commands.iter().enumerate() {
            if i == selection_index {
                ...
                context.rectangle(0.0, item_height * (i + 1) as f64, window_width, item_height);

                context.fill().expect("Unable to draw selection box.");
